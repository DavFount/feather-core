...
RegisterCommand('CoreAccountSettingsSmokeTest', function(source, args)
    ...
    local accountId = account.ok and account.value and account.value.accountId or nil
    local original = accountId and CoreAccountSettings.Get(accountId) or nil
    local originalLocale = original and original.ok and original.value and original.value.locale or nil
    local persisted = accountId and originalLocale
        and CoreAccountSettings.Set(accountId, originalLocale) or nil
    local reread = accountId and CoreAccountSettings.Get(accountId) or nil
    ...
    local tests = {
        ...
        {
            name = 'registered locale',
            passed = type(originalLocale) == 'string'
                and type(LocalesAPI.translations[originalLocale]) == 'table'
        },
        {
            name = 'persistence round trip',
            passed = persisted and persisted.ok == true
                and reread and reread.ok == true and reread.value
                and reread.value.locale == originalLocale
        },
        {
            name = 'invalid locale rejected',
            passed = accountId
                and CoreAccountSettings.Set(accountId, '__invalid__').code == 'locale_invalid'
        },
        {
            name = 'versioned routes',
            passed = routeNames['core.account.settings.get.v1'] == true
